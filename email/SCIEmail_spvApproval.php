...
$mail = new \PHPMailer\PHPMailer\PHPMailer();
				...
				$mail->Subject = '[BIPH_PDAU] Request for Approval - '.$RequestType.' ['. $requestID .']';
				...
				$body = '<p style="font-size:18px;">Dear '.$MGR.',</p>
				<p style="font-size:18px;">Good Day!</p>
				<p style="font-size:18px;">'.$Requestor.' from '.$RequestSection.', request for '.$RequestType.' SCI with RequestID ('.$requestID.') .</p>
				...
				<td style="border:1px solid #dddddd; text-align:left;padding:8px;">'.$sciFinalSPV.'</td>
				...
				$sql = "SELECT EmailAddress FROM Accounts WHERE FullName = '$MGR'";
				$stmt = sqlsrv_query($conn2,$sql);
				while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
					try {
						$mail->addAddress($row['EmailAddress'], $MGR);
					} catch (Exception $e) {
